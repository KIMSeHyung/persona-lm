...
- 빌드 후에는 실행형 CLI `personalm`으로 persona mirror launcher를 실행할 수 있다.
...
소스 상태에서 바로 실행할 때는 `pnpm persona:mirror`를 사용한다.
...
`dev_feedback`는 항상 피드백을 묻는 모드는 아니다.
답변 품질 보강이 필요하다고 판단될 때만 짧게 score/reason 피드백을 요청하고, 그 결과를 `submit_feedback`에 연결하는 실험 모드다.

빌드 후 실행형 CLI를 쓰고 싶다면 다음처럼 준비할 수 있다.
...
그 뒤에는 `personalm` 명령으로 실행한다.

```bash
personalm mirror --mode locked "콘텐츠 버전 관리 관련 의사결정 성향을 말해줘"
personalm mirror --mode dev_feedback "동시성과 idempotency를 어떻게 판단하는 편인지 말해줘"
```

`pnpm link --global` 대신 직접 symlink를 만들고 싶다면 `bin/personalm`을 `PATH`에 연결하면 된다.
`personalm`은 두 경로를 분리해서 본다.

- Codex가 실제로 열릴 작업 프로젝트: 현재 디렉터리 또는 `-C`
- `persona_lm` MCP backend와 instruction asset 경로: `PERSONALM_HOME`

다른 프로젝트에서 자주 실행할 거라면 `PERSONALM_HOME`을 `persona-lm` 폴더로 고정한다.

```bash
export PERSONALM_HOME=/Users/gimsehyeong/persona-llm
cd /path/to/other-project
personalm mirror --mode locked "콘텐츠 버전 관리 관련 의사결정 성향을 말해줘"
```

이 경우 Codex는 `/path/to/other-project`에서 열리고, `persona_lm` MCP는 `/Users/gimsehyeong/persona-llm`을 바라본다.

특정 실행에서만 다른 작업 프로젝트를 쓰고 싶다면 `-C`로 workspace를 넘긴다.

```bash
personalm mirror -C /path/to/other-project --mode locked "질문"
```

`PERSONALM_HOME`이 없으면 `personalm`을 실행한 현재 셸 디렉터리를 backend root로도 같이 사용한다. 즉 `persona-lm` repo 안에서 실행할 때는 추가 설정 없이 기존처럼 동작한다.
...
기본적으로는 세션 종료만 기다리지 않고, 하나의 대화 단위에서 durable한 성향이나 작업 방식이 충분히 드러났다고 판단되면 저장을 시도하도록 instruction이 들어가 있다.
명시적으로 저장을 유도하고 싶다면 대화 중간이나 마지막에 아래처럼 요청하면 된다.
...
durable한 내용이 없거나 사용자가 저장하지 말라고 한 경우에는 저장하지 않는다.
